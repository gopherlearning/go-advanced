...
func (s *MetricStore) MemStats() []string {
	s.mu.RLock()
	res := make([]string, 0)
...
func (s *MetricStore) Custom() map[string]Metric {
	s.mu.Lock()
...
func (s *MetricStore) Save(client *http.Client, baseURL *string) error {
	res := s.All()
	fmt.Println(res)
...
func (s *MetricStore) AddCustom(m ...Metric) {
	s.mu.Lock()
	for _, v := range m {
...
func (s *MetricStore) Scrape() error {
	s.mu.Lock()
	defer s.mu.Unlock()
